...
if (!empty($options["callme_api_url"]) && !empty($options["callme_api_key"]) && !empty($options["api_mediaId"]) && !empty($options["api_channel"])) {
    ...
    function escale_callme_load_jquery() {
        ...
        wp_register_style( 'escale_callme', plugins_url('/css/escale_callme.css', __FILE__), false, '1.0.4', 'all');
        ...
        wp_register_script( 'escale_callme', plugins_url('/js/escale_callme.js',__FILE__ ), false, '1.0.10', 'all');
